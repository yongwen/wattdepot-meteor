...
var getAggregationGroupId = function (step) {
    ...
    if (stepType != 'year') _id.year = {$year: "$timeStamp"};
    else {
        _id.year = {$subtract: [
            {$year: "$timeStamp"},
    ...
    if (stepType != 'month') _id.month = {$month: "$timeStamp"};
    else {
        _id.month = {
            $subtract: [
                {$month: "$timeStamp"},
    ...
    if (stepType != 'day') _id.day = {$dayOfMonth: "$timeStamp"};
    else {
        _id.day = {$subtract: [
    ...
    if (stepType != 'hour') _id.hour = {$hour: "$timeStamp"};
    else {
        _id.hour = {$subtract: [
            {$hour: "$timeStamp"},
...
getStartEndTimeStampValue = function(query, sensorId) {
    ...
    if (limit)
        ...
        option.limit = 100;  // default limit to 100 if not specified

    // fix the bound
    var ts_start = query.ts_start, ts_end = query.ts_end;
    if (!ts_end && ts_start) ts_end = new Date();
    if (!ts_start && ts_end) ts_start = new Date();
